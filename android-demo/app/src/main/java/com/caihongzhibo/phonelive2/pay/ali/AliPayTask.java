package com.caihongzhibo.phonelive2.pay.ali;
...
public class AliPayTask {
    ...
    public AliPayTask(Activity activity, ChargeBean bean, final PayCallback callback) {
        mActivity = activity;
        mBean = bean;
        ...
        mHandler = new Handler() {
            public void handleMessage(Message msg) {
                ...
                if ("9000".equals(result.get("resultStatus"))) {
                    if (callback != null) {
                        callback.onSuccess(mBean.getCoin() + mBean.getGive());
                    }
                } else {
                    if (callback != null) {
                        callback.onFailuer();
