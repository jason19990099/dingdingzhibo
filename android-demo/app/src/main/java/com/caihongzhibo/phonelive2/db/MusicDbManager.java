package com.caihongzhibo.phonelive2.db;

import android.database.Cursor;
import android.database.sqlite.SQLiteDatabase;
import android.database.sqlite.SQLiteStatement;

import com.caihongzhibo.phonelive2.bean.LiveMusicBean;
...
public class MusicDbManager {
    ...
    public void save(LiveMusicBean bean) {
        ...
        if (db.isOpen()) {
            Cursor c = db.rawQuery(MusicDbHelper.QUERY + "'" + bean.getAudio_id() + "'", null);
            if (c.moveToNext()) {//如果存在，执行update
                SQLiteStatement st = db.compileStatement(MusicDbHelper.UPDATE);
                db.beginTransaction();
                try {
                    st.bindString(1, bean.getAudio_name());
                    st.bindString(2, bean.getArtist_name());
                    st.bindString(3, bean.getAudio_id());
                ...
                db.beginTransaction();
                try {
                    st.bindString(1, bean.getAudio_id());
                    st.bindString(2, bean.getAudio_name());
                    st.bindString(3, bean.getArtist_name());
                    st.executeInsert();
                    db.setTransactionSuccessful();
                } finally {
                    db.endTransaction();
        ...
        if (db.isOpen()) {
            ...
            try {
                ...
                while (c.moveToNext()) {
                    ...
                    bean.setAudio_id(c.getString(c.getColumnIndex(MusicDbHelper.ID)));
                    bean.setAudio_name(c.getString(c.getColumnIndex(MusicDbHelper.NAME)));
                    bean.setArtist_name(c.getString(c.getColumnIndex(MusicDbHelper.ARTIST)));
                    bean.setStatus(LiveMusicBean.PRO_END);
                    list.add(bean);
                }
            } finally {
                if (c != null) {
