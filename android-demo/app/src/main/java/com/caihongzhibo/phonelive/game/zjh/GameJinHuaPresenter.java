package com.caihongzhibo.phonelive.game.zjh;
...
public class GameJinHuaPresenter extends AbsGamePresenter {

    private GameJinHuaFragment mJinHuaFragment;
    private UserBean mUser;

    public GameJinHuaPresenter(Context context, FragmentManager fragmentManager, String stream, String liveuid) {
    ...
    private void onOpenGameWindow(int startType, boolean enterRoom, int betCount, int[] gameTotalBet, int[] gameMyBet) {
        if (mJinHuaFragment != null) {
            mJinHuaFragment.closeGame();
            mJinHuaFragment = null;
        }
        mJinHuaFragment = new GameJinHuaFragment();
        ...
        mJinHuaFragment.setArguments(bundle);
        mJinHuaFragment.setGamePresenter(this);
        FragmentTransaction ft = mFragmentManager.beginTransaction();
        ft.replace(R.id.repalced_game, mJinHuaFragment).commit();
    ...
    public void gameCreate() {
        if (mIsAnchor && !mGameClosed) {
            if (mSocketConn) {
                HttpUtil.gameJinhuaCreate(mStream, mGameCreateCallback);
            } else {
    ...
    private void onStartBet(JSONObject obj) {
        ...
        mGameToken = obj.getString("token");
        mBetTime = obj.getIntValue("time");
        mJinHuaFragment.startBet(mBetTime - 1);
    }
    ...
    public void gameBet(final String coin, final String grade) {
        if (!mGameClosed) {
            HttpUtil.gameJinhuaBet(mGameId, coin, grade, new HttpCallback() {
                @Override
                public void onSuccess(int code, String msg, String[] info) {
                    if (code == 0) {
                        JSONObject obj = JSON.parseObject(info[0]);
                        mJinHuaFragment.setLastCoin(obj.getString("coin"));
                        sendBet(coin, grade);
                    } else {
                        ToastUtil.show(msg);
                    }
    ...
    private void onBet(JSONObject obj) {
        ...
        mJinHuaFragment.onBet(index, money, isSelf);
    }
    ...
    private void onShowResult(String[][] result) {
        ...
        mJinHuaFragment.showResult(result);
    }
        ...
        public void onSuccess(int code, String msg, String[] info) {
            if (code == 0) {
                ...
                if (gameCoin > 0) {
                    ...
                    mJinHuaFragment.onSettle(coin, gameCoin);
                }
            } else {
                ToastUtil.show(msg);
            }
    ...
    public boolean anchorCloseGame() {
        if (mIsAnchor) {
            if (mSocketConn) {//socket连接了
                if (mJinHuaFragment.getStatus() == GameJinHuaFragment.STATUS_READY || mCanCloseGame && mJinHuaFragment.getBetCount() == 0) {
                    SocketUtil.getInstance().sendSocketMessage(new SendSocketBean()
                            .param("_method_", GameConst.SOKCET_GAME_JIN_HUA)
                            .param("action", 3)
                            .param("msgtype", 15)
                ...
                if (mJinHuaFragment.getBetCount() == 0) {
                    onCloseGame();
                    return true;
                } else {
                    ToastUtil.show(WordUtil.getString(R.string.game_please_wait_game_end));
    ...
    private void onCloseGame() {
        ...
        mJinHuaFragment.closeGame();
        FragmentTransaction ft = mFragmentManager.beginTransaction();
        ft.remove(mJinHuaFragment).commit();
        mJinHuaFragment = null;
    ...
    public boolean anchorCanReplaceGame() {
        ...
        if (mIsAnchor) {
            if (mJinHuaFragment.getStatus() == GameJinHuaFragment.STATUS_READY) {
                return true;
            } else {
                ToastUtil.show(WordUtil.getString(R.string.game_please_wait_game_end));
            }
